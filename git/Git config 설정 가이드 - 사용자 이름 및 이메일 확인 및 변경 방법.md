...
전역 설정과 로컬 설정 중 무엇을 쓸지는 아래 기준으로 판단하면 됩니다.

```mermaid
flowchart TD
    A["이 프로젝트에 다른 이름/이메일이<br/>필요한가?"] -->|아니오| B["--global 옵션 사용<br/>(모든 저장소에 공통 적용)"]
    A -->|예| C["옵션 없이 로컬 설정<br/>(해당 저장소에만 적용)"]
    B --> D["~/.gitconfig 에 저장"]
    C --> E["해당 저장소의 .git/config 에 저장"]
```
...
```mermaid
flowchart LR
    S["System<br/>(/etc/gitconfig)"] --> G["Global<br/>(~/.gitconfig)"] --> L["Local<br/>(.git/config)"]
    L -->|최종 적용| R["실제 커밋에 사용되는 값"]
```

우선순위가 낮은 System, Global 설정값은 더 우선순위가 높은 Local 설정이 존재할 경우 덮어써집니다.
