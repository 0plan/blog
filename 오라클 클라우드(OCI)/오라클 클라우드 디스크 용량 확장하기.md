...
<img src="https://github.com/0plan/blog/blob/main/assets/images/post-cards/오라클-클라우드-디스크-용량-확장하기-720256.png?raw=true" alt="오라클 클라우드 Ubuntu 디스크 용량 확장 가이드" />
...
<!-- TODO: 실제 스크린샷 추가 필요 (예: OCI 콘솔 부트 볼륨 크기 조정 화면) -->
...
```mermaid
flowchart TD
    A[OCI 콘솔에서 부트 볼륨 크기 증가] --> B[인스턴스 SSH 접속]
    B --> C[디스크 재스캔]
    C --> D[df -h로 현재 상태 확인]
    D --> E[growpart로 파티션 확장]
    E --> F[resize2fs로 파일 시스템 확장]
    F --> G[df -h로 확장 결과 확인]
```
