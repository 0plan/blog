...
```mermaid
flowchart TD
    A["1단계: jdeps로 내부 API 사용 여부 분석"] --> B["2단계: 빌드 도구 및 라이브러리 업데이트"]
    B --> C["3단계: 삭제된 J2EE 모듈 의존성 추가"]
    C --> D["4단계: --release 옵션으로 컴파일러 설정 변경"]
    D --> E["5단계: 런타임/리플렉션 오류 해결"]
    E --> F["6단계: OpenRewrite로 자동 리팩토링"]
    F --> G["점진적 도입: 새로운 문법 적용"]
```
