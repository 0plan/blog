...
<img src="https://github.com/0plan/blog/blob/main/assets/images/post-cards/2-Spring-Boot-프로젝트-시작하기-88b4bc.png?raw=true" alt="Spring Boot 프로젝트 시작하기: 첫 번째 REST API 만들기" />
...
### **설정 권장값:**
*   **Project:** **Gradle - Groovy** (가장 대중적) 혹은 **Maven**
*   **Language:** Java
*   **Spring Boot:** 3.4.x (안정 버전 중 최신)
*   **Java:** 21 (LTS 버전 권장)
*   **Dependencies:** `Spring Web`, `Lombok`, `Spring Boot DevTools` 추가

<!-- TODO: 실제 스크린샷 추가 필요 (예: Spring Initializr 설정 화면) -->

---

## 2. 빌드 도구별 의존성 설정 (build.gradle vs pom.xml)

프로젝트가 생성되면 각 빌드 도구에 맞는 설정 파일이 생깁니다.
...
요청이 들어왔을 때 내부적으로 어떤 순서로 처리되는지 그림으로 보면 다음과 같습니다.

```mermaid
sequenceDiagram
    participant Browser as 브라우저
    participant Tomcat as 내장 Tomcat
    participant Controller as HelloController
    Browser->>Tomcat: GET /hello
    Tomcat->>Controller: sayHello() 호출
    Controller-->>Tomcat: "Hello, Spring Boot 3 with Java 21!"
    Tomcat-->>Browser: 200 OK 응답
```
