...
<img src="https://github.com/0plan/blog/blob/main/assets/images/post-cards/4-스토리지-서버-설치하기-87d9c2.png?raw=true" alt="스토리지 서버" />
...
```mermaid
flowchart TD
    A[1. 시스템 요구 사항 확인] --> B[2. 필요한 패키지 설치]
    B --> C[3. 데이터베이스 설정]
    C --> D[4. Nextcloud 다운로드]
    D --> E[5. Apache/Nginx 설정]
    E --> F[6. Nextcloud 웹 설치]
    F --> G[7. SSL 설정 - 선택 사항]
```
...
웹 서버와 PHP, 데이터베이스는 아래와 같은 흐름으로 요청을 처리합니다.

```mermaid
flowchart LR
    U[사용자 브라우저] -->|HTTP/HTTPS 요청| W[Apache 또는 Nginx]
    W -->|PHP 요청 전달| P[PHP-FPM / mod_php]
    P -->|파일·설정 조회| N[Nextcloud 애플리케이션]
    N -->|쿼리| DB[(MySQL/MariaDB)]
    DB -->|결과 반환| N
    N -->|응답| P
    P -->|응답| W
    W -->|HTML 응답| U
...
<!-- TODO: 실제 스크린샷 추가 필요 (예: Nextcloud 웹 설치 마법사 초기 화면) -->
